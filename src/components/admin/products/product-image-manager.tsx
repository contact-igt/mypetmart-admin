...
export function ProductImageManager({ productId, images = [], pending, onPendingChange, onChanged }: {
  ...
  return <section className="rounded-xl border border-border-subtle bg-white p-4 sm:p-5">
    ...
    <div className="mt-4 grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">{ordered.map((image,index) => <article key={image.id} className="rounded-lg border border-border-subtle p-3">
      <img src={image.url} alt={image.alt} className="aspect-square w-full max-h-32 rounded-lg bg-cream-bg object-cover" />
    ...
    <MediaPickerDrawer open={galleryOpen} onClose={() => setGalleryOpen(false)} onSelect={selectGalleryAsset} allowedTypes={["image"]} />
  </section>;
}
